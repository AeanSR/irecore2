...
namespace
{
    ...
    std::string (*fp_dumpCallback) (void) = NULL;
...
#undef EXC_CASE
	...
	void WriteBlackBoxMessages(FILE* f)
	{
		...
		{
			int threadId;
			const char* message = rde::BlackBox::GetMessage(i, threadId);
	...
	LONG __stdcall MyExceptionFilter(EXCEPTION_POINTERS* exceptionPtrs)
	{
        ...
        std::string cbdump = (*fp_dumpCallback)();
        fprintf(f, "IreCore Dump\n============\n%s\n", cbdump.c_str());
...
void CrashHandler::SetDumpCallback(std::string (* pcb)(void))
{
    fp_dumpCallback = pcb;
}
