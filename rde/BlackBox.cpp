...
#if RDE_BLACKBOX_STANDALONE
#	define WIN32_LEAN_AND_MEAN
#	define NOMINMAX
#	include <windows.h>
#	undef GetMessage	// overrides BlackBox::GetMessage...
...
#	define RDE_GET_CURRENT_THREAD_ID	rde::Thread::GetCurrentThreadId
#endif
...
#else
	rde::ScopedPtr<BlackBoxEntry, rde::ArrayDeleter>	s_messages(0);
	// Must be power-of-two.
	RDE_COMPILE_CHECK((kMaxMessages & (kMaxMessages - 1)) == 0);
#endif
...
void BlackBox::AddMessagef(const char* fmt, ...)
{
	if (s_enabled)
	{
		...
		_vsnprintf(buff, sizeof(buff) - 1, fmt, args);
		va_end(args);
		AddMessage(buff);
	}
}
#endif // RDE_BLACKBOX_ENABLED

long BlackBox::GetNumMessages()
{
	return s_overflow ? kMaxMessages : s_topMessageIndex;
}
const char* BlackBox::GetMessage(long index, int& threadId)
{
	if (s_dirty)
	{
