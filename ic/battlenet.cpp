...
int gic::import_player( std::string& realm, std::string& name, std::string& region, int silence ) {
    ...
    for ( int i = 0; i < 16; i++ ) {
        ...
        if ( gemid ) {
            ...
            switch ( ench->stat ) {
			case 71:
            case 4: case 72: case 74: paperdoll.gear_list[i].str += ench->value; break;
            case 32: paperdoll.gear_list[i].crit += ench->value; break;
            case 36: paperdoll.gear_list[i].haste += ench->value; break;
            case 49: paperdoll.gear_list[i].mastery += ench->value; break;
            case 59: paperdoll.gear_list[i].mult += ench->value; break;
            case 40: paperdoll.gear_list[i].vers += ench->value; break;
            default: break;
            }
        }
        ...
        if ( enchid ) {
            ...
            switch ( ench->stat ) {
			case 4: case 71: case 72: case 74: paperdoll.gear_list[i].str += ench->value; break;
            case 32: paperdoll.gear_list[i].crit += ench->value; break;
            case 36: paperdoll.gear_list[i].haste += ench->value; break;
            case 49: paperdoll.gear_list[i].mastery += ench->value; break;
            case 59: paperdoll.gear_list[i].mult += ench->value; break;
        ...
        if ( plate_slot[i] ) {
            paperdoll.gear_list[i].type = 1;
            for ( int* p = plate_list; *p; p++ ) {
                if ( *p == itemid ) {
                    paperdoll.gear_list[i].type = 0;
                    break;
                }
            }
        }
