...
QString script_scale_factor(
    ...
    script += QString("-- ") + QApplication::translate("script", "Use a bar chart to visualize the results.") + "\n";
    script += "delta = ";
...
QString script_contour_plot(
    ...
    const char* stat_name[] = { "crit", "haste", "mastery", "mult", "vers" };
    QString script;
    script += QString("-- ") + QApplication::translate("script", "This script is generated by IreCore.") + "\n";
    script += "Silence(true)\n";
    script += "delta = ";
    ...
    script += "stat = {}\n";
    script += QString("stat[1] = GetParam(\"gear_") + stat_name[stat1] + "\")\n";
    script += QString("stat[2] = GetParam(\"gear_") + stat_name[stat2] + "\")\n";
    script += QString("stat[3] = GetParam(\"gear_") + stat_name[stat3] + "\")\n";
    script += "amount = stat[1] + stat[2] + stat[3]\n";
    script += "function RunPlot(s,e,m)\n";
    script += QString("  SetParam(\"gear_") + stat_name[stat1] + "\", s[1])\n";
    script += QString("  SetParam(\"gear_") + stat_name[stat2] + "\", s[2])\n";
    script += QString("  SetParam(\"gear_") + stat_name[stat3] + "\", s[3])\n";
    ...
    script += "    error = math.sqrt((error * error * iter * iter + more_error * more_error * more_iter * more_iter)) / (iter + more_iter)\n";
    ...
    script += "data_count = 0\n";
    script += "for stat[1] = 0, amount, delta do\n";
    script += "  for stat[2] = 0, amount, delta do\n";
    script += "    stat[3] = amount - stat[1] - stat[2]\n";
    script += "    if stat[3] >= 0 then\n";
    script += "      dps, dpse = RunPlot(stat, tolerance, max_iteration)\n";
    script += "      print(stat[1],stat[2],stat[3],string.format(\"%.1f\",dps),string.format(\"%.3f\",dpse))\n";
    script += "      io.write(stat[1]..\", \"..stat[2]..\", \"..string.format(\"%.3f\",dps)..string.format(\"%.3f\",dpse)..\"\\n\")\n";
